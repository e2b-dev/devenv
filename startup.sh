...
su ${username} -c "source /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh && NIXPKGS_ALLOW_UNFREE=1 nix profile install --impure nixpkgs#git nixpkgs#go nixpkgs#pnpm_9 nixpkgs#python3 nixpkgs#nodejs nixpkgs#opentofu nixpkgs#google-cloud-sdk nixpkgs#tailscale nixpkgs#git-credential-oauth nixpkgs#gcc"
...
su ${username} -c "source /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh && git clone https://github.com/e2b-dev/devenv /home/${username}/devenv"